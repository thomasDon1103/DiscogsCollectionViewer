  ...
  <div class="relative">
    ...
    >
      <div v-if="showCollection" class="flex items-center justify-center gap-4 mb-10">
        <button 
          @click="resetForm"
          class="flex items-center gap-2.5 px-5 py-2.5 bg-white/10 backdrop-blur-sm text-white/80 rounded-full text-sm font-medium border border-white/10 hover:bg-white/20 hover:text-white hover:border-white/20 active:scale-95 transition-all duration-300"
        >
        ...
        <div class="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-10 shadow-2xl shadow-black/20">
          ...
          <form @submit.prevent="handleSubmit" class="space-y-5">
            <div class="space-y-1.5">
              <label class="block text-left text-white/60 text-xs font-semibold uppercase tracking-wider pl-1">Username</label>
              <div class="relative">
                ...
                <input
                  ...
                  type="text"
                  placeholder="e.g. vinylcollector"
                  required
            ...
            <div class="space-y-1.5">
              <label class="block text-left text-white/60 text-xs font-semibold uppercase tracking-wider pl-1">API Key <span class="text-white/30 normal-case font-normal">(optional)</span></label>
              <div class="relative">
                <div class="absolute left-4 top-1/2 -translate-y-1/2 text-white/30">
                  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                ...
                <input
                  ...
                  type="text"
                  placeholder="Your Discogs API key"
                  class="w-full pl-12 pr-5 py-4 text-base rounded-xl text-white placeholder-white/25 bg-white/5 border border-white/10 focus:bg-white/10 focus:border-primary-start/60 focus:shadow-[0_0_20px_rgba(102,126,234,0.15)] transition-all duration-300 outline-none"
                />
              </div>
            </div>
      ...
      <div v-if="showCollection" class="max-w-7xl mx-auto">
        ...
        <div class="mb-10">
          ...
          <p class="text-white/40 text-sm">
            <span v-if="loadingMore" class="inline-flex items-center gap-2">
              <svg class="animate-spin h-3.5 w-3.5 text-white/40" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
              </svg>
        ...
        <div class="px-4 py-6">
          <div class="flex items-center justify-center gap-6">
            ...
            <div 
              class="relative flex-1 max-w-5xl overflow-visible cursor-grab active:cursor-grabbing select-none"
            ...
            >
              <div class="flex items-center justify-center relative" style="height: 580px;">
                <div 
                  v-for="(release, index) in collectionData!.releases" 
                  :key="release.id"
                  class="absolute transition-all duration-500 ease-in-out"
                  :style="getCardStyle(index)"
                >
                  <div 
                    class="rounded-2xl overflow-hidden card-glass"
                    :class="{
                  ...
                  >
                    <div class="relative">
                      ...
                      <div v-if="index === currentIndex" class="absolute bottom-0 left-0 right-0 h-16 bg-gradient-to-t from-black/30 to-transparent"></div>
                      <!-- Format badge -->
                    ...
                    <div 
                      ...
                      class="p-6 bg-surface-light/95 backdrop-blur-sm"
                    >
                      <p class="font-bold text-xl text-white mb-1.5 leading-tight truncate">{{ release.basic_information.title }}</p>
                      <p class="text-white/60 text-base mb-4 truncate">{{ release.basic_information.artists.map(a => a.name).join(', ') }}</p>
            ...
            <button 
              @click="nextRelease"
              :disabled="currentIndex === collectionData!.releases.length - 1"
              class="shrink-0 w-12 h-12 flex items-center justify-center bg-white/10 backdrop-blur-sm text-white rounded-full border border-white/10 hover:bg-white/20 hover:border-white/20 hover:scale-110 active:scale-95 transition-all duration-300 disabled:opacity-20 disabled:cursor-not-allowed disabled:hover:scale-100 disabled:hover:bg-white/10 z-10"
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div class="flex items-center justify-center gap-3 mt-4">
          ...
          <div class="w-48 h-1 bg-white/10 rounded-full overflow-hidden relative">
            ...
            <div 
              ...
              :style="{ width: `${(collectionData!.releases.length / totalItems) * 100}%` }"
            ></div>
